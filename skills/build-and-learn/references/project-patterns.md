...
**React 常见教学要点**：JSX 不是 HTML 是 JavaScript 表达式；组件名必须大写；`useState` 不可变性（不要直接改 state）；为什么需要 key；受控组件 vs 非受控组件。
...
**Vue 常见教学要点**：SFC 的三个块（template/script/style）；`ref()` vs `reactive()`；`v-model` 是语法糖；模板中访问 ref 不用 `.value`。
...
**后端常见教学要点**：什么是 API/REST；GET/POST/PUT/DELETE；状态码 200/201/400/404/500；`req.params` vs `req.query` vs `req.body`。
...
**全栈特别注意**：一定要讲 CORS 是什么、为什么需要；前端请求的 URL 在开发时和部署后不一样；后端报错前端怎么展示。
...
类比盖楼的脚手架：先搭骨架，再填充细节，每次只改一个部分。

**三个不跳**：不跳概念（每个新概念都完整解释）、不跳代码（每行代码都解释）、不跳验证（每步都让用户跑一遍确认）。

### 示例：教 React 组件

❌ 不要一次给出最终版本的完整组件（所有 state、事件、JSX 一起上）

✅ 分步来：第 1 步最简组件只显示静态内容 → 第 2 步加一个 state → 第 3 步加事件 → 以此类推
...
| 直接修改 state | `items.push(x)` 而非 `setItems([...items, x])` | 讲不可变性原理 |
...
什么概念在项目的哪个阶段讲最合适？**决策原则**：不要提前讲 —— 在用户「需要它」的那一刻再讲。
