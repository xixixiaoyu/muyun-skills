...
**上下文识别**：
- 先审查变更文件列表，识别实际涉及的语言和框架（通过文件扩展名、导入路径、框架特征）。
- 根据变更文件中实际出现的语言/框架，按需加载对应的框架检查清单（`frameworks/` 目录）。仅加载变更涉及的技术栈，不加载项目整体但本次未变更的框架。
- 对 monorepo 项目，锁定变更影响的子包/工作区，避免全量加载所有子包的检查清单。
